...
import { AllocationStrategy, Prize, State } from "../lib/types";
...
const PrizePage: React.FC = () => {
  ...
  if (rolesError) {
    console.error("Error fetching roles:", rolesError);
    return <div className="text-center py-10 text-red-300">Error loading roles: {(rolesError as Error).message}</div>;
  }
        ...
        <div className="p-6 space-y-8">
          {isConnected && (
            <div className="bg-purple-50 rounded-lg p-4">
              ...
              <div className="flex flex-wrap gap-4">
                {roles?.canSubmit && (
                  <button className="button-primary" onClick={() => navigate(`/prize/${prizeId}/submit`)}>
                    Submit Contribution
                  </button>
                )}
                {roles?.canEvaluate && (
                  <button className="button-primary" onClick={() => navigate(`/prize/${prizeId}/evaluate`)}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-2">
              <PrizeDetails prize={displayPrize} />
...
const PrizeAmount: React.FC<{ amount: bigint }> = ({ amount }) => {
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <p className="text-3xl font-bold text-purple-600">{formatEther(amount || 0n)} ETH</p>
    </div>
  );
};
...
const PrizeInPageStatus: React.FC<{ currentState: State }> = ({ currentState }) => {
  const stateLabels: Record<State, string> = {
    [State.Setup]: "Setup",
    [State.Funded]: "Funded",
    [State.WeightsAssigned]: "Weights Assigned",
    [State.EvaluatorsAssigned]: "Evaluators Assigned",
  ...
  const statusColors: Record<State, string> = {
    ...
    [State.Funded]: "bg-green-100 text-green-800",
    [State.WeightsAssigned]: "bg-blue-100 text-blue-800",
    [State.EvaluatorsAssigned]: "bg-purple-100 text-purple-800",
          ...
          <div
            ...
            className={`flex items-center p-2 rounded-md ${
              currentState === Number(state)
                ? `${statusColors[Number(state)]} font-semibold`
                : "bg-gray-50 text-gray-500"
            }`}
          >
            <div
              className={`w-3 h-3 rounded-full mr-2 ${currentState === Number(state) ? "bg-current" : "bg-gray-300"}`}
            ></div>
            <span>{label}</span>
          </div>
        ))}
...
const PrizeStatistics: React.FC<{ prize: Prize }> = ({ prize }) => {
  return (
    <div className="bg-white rounded-lg shadow p-6 mt-6">
      <h2 className="text-2xl font-semibold text-purple-800 mb-4">Prize Statistics</h2>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <p className="text-sm font-medium text-gray-500">Contribution Count</p>
          <p className="text-lg font-semibold text-gray-800">{prize.contributionCount.toString()}</p>
        </div>
        <div>
          <p className="text-sm font-medium text-gray-500">Evaluated Contributions</p>
          <p className="text-lg font-semibold text-gray-800">{prize.evaluatedContributionsCount.toString()}</p>
        </div>
        <div>
          <p className="text-sm font-medium text-gray-500">Claimed Rewards</p>
          <p className="text-lg font-semibold text-gray-800">{prize.claimedRewardsCount.toString()}</p>
        </div>
        <div>
          <p className="text-sm font-medium text-gray-500">Rewards Allocated</p>
          <p className="text-lg font-semibold text-gray-800">{formatEther(prize.rewardsAllocated)} ETH</p>
        </div>
      </div>
    </div>
  );
...
const generateGradient = (prize: Prize) => {
  const colors = [
    "#FF5733",
    "#FFBD33",
    "#FFF133",
