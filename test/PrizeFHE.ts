...
describe.only("PrizeFHE", function () {
  ...
  describe("Prize Lifecycle", function () {
    it("should simulate a complete prize lifecycle with FHE", async function () {
      ...
      const encryptedRewardPool = await client.encrypt_uint32(totalRewardPool);
      ...
      await prizeContract.connect(organizer).fundPrize(encryptedRewardPool, { value: totalRewardPool });

      const sealedBalanceAfterFunding = await prizeContract.getEncryptedBalance(permission);
      const decryptedBalanceAfterFunding = await client.unseal(prizeAddress, sealedBalanceAfterFunding);
      expect(decryptedBalanceAfterFunding).to.equal(totalRewardPool);
      ...
      const encryptedScores1 = [
        await client.encrypt_uint32(80),
        await client.encrypt_uint32(70),
        await client.encrypt_uint32(90)
      ];
      const encryptedScores2 = [
