...
export function generateRandomPrizeParams(seed: string = DEFAULT_TEST_SEED): PrizeParams {
  ...
  const criteria = Array.from(
    ...
    () => CRITERIA_OPTIONS[Math.floor(Math.random() * CRITERIA_OPTIONS.length)],
  );
...
export async function extractPrizeIdFromTx(contract: DiamondWithFacets, tx: TransactionResponse): Promise<bigint> {
  const receipt = await tx.wait();
  if (!receipt) throw new Error("Transaction receipt not found");

  let prizeId: bigint | null = null;

  for (const log of receipt.logs) {
    try {
      const parsedLog = contract.interface.parseLog(log);
      if (parsedLog && parsedLog.name === "PrizeCreated") {
        prizeId = BigInt(parsedLog.args.prizeId.toString());
        break;
      }
    } catch (error) {
      // This log does not belong to the contract, ignore
      continue;
    }
  }
...
export async function connectDiamond(signer: Signer): Promise<DiamondWithFacets> {
  ...
  for (const facetName of FACET_NAMES) {
    try {
      const facetArtifact = await artifacts.readArtifact(facetName);
      combinedABI = combinedABI.concat(facetArtifact.abi);
    } catch (error) {
      console.error(`Error reading ABI for facet ${facetName}:`, error);
      throw error;
    }
  }
...
export async function getFacetName(diamondLoupeFacet: DiamondLoupeFacet, facetAddress: string): Promise<string> {
  ...
  for (const name of FACET_NAMES) {
    try {
      ...
      console.log(
        `Error matching ${name} to ${facetAddress}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
...
export async function logTransaction(
  ...
  if (!receipt) {
    throw new Error(`${operation} - Receipt not available`);
  }
